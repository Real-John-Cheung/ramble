...
const stops = ["also", "over", "have", "this", "that", "just", "then", "under", "some",
  // added: DCH
  'these',
  "within",
  "after",
...
Object.keys(history).map(k => sources[k].map((w,i) => history[k][i] = [w])); // only repIds
console.log(history);
//console.log(repIds.length, strictRepIds.length);
...
spanify(state.destination);
let data1 = replace(state.destination);
let data2 = replace(shadow(), data1.idx);
...
function replace(textName, inIdx) {
  ...
  if (typeof inIdx !== 'undefined') {
    ...
    choices = RiTa.randomOrdering(repIds);
  }
  ...
  for (let i = 0; i < choices.length; i++) {
    ...
    idx = choices[i];
    pos = sources.pos[idx];
    word = sources[textName][idx];
    if (!word) throw Error('Replace Error: ' + this.name
      + ' for index=' + this.idx + '\n' + JSON.stringify(choices));
    word = word.toLowerCase();

    let sims = similars(word, pos);
    if (!sims) continue;

    // pick a random similar
    next = RiTa.random(sims);

    //this.words[idx] = next; // do replacement
    history[textName][idx].push(next); // add to history
...
function spanify(data) {
  console.log(data);
  data = Array.isArray(data) ? data : sources[data];
  console.log(data);
